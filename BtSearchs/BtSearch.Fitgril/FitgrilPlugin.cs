...
public class FitgrilPlugin : IBTSearchPlugin
{
    ...
    public async IAsyncEnumerable<BTSearchResult> SearchAsync(string query, CancellationToken token = default)
    {
        ...
        foreach (var node in nodes)
        {
            ...
            result.WebUrl = href;
            var session = await HttpClientProvider.Client.GetAsync(href);
            #region 获取图片
            ...
            #endregion
            #region 获得下载链接
            var entrycontent = articledoc.DocumentNode.SelectSingleNode("//a[text()='magnet']/@href");
